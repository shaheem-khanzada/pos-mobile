...
import { useEditProductMutation } from '@/hooks/use-products-mutations';
...
export function EditProductScreen({ product }: EditProductScreenProps) {
  ...
  const isSubmitting = editProductMutation.isPending;
  const imagePreviewUrl = withMediaBaseUrl(selectedImage?.uri || getProductImageUrl(product.image));
    ...
    <SafeAreaView className="flex-1 bg-background-50" edges={['top', 'left', 'right', 'bottom']}>
      <VStack className="flex-1 px-4 pt-2">
        ...
        <Button className="mb-5 mt-auto h-14 rounded-full bg-primary-600" onPress={handleSubmit(onSubmit)} isDisabled={isSubmitting}>
          <ButtonText className="text-lg font-medium text-typography-0">{isSubmitting ? 'Saving...' : 'Save Changes'}</ButtonText>
        </Button>
      </VStack>
    </SafeAreaView>
  );
